from tiktaktoe.krest_and_nol import Game
from tiktaktoe.board import X, O
from tiktaktoe.player import HumanPlayer, RoboticPlayer
import random
...
def main():
    ...
    while True:
        choice_you = choice()
        if choice_you == 1:
            with open("tiktaktoe/rules_of_the_game", "r", encoding='utf-8') as file:
                rules_the_game = file.read()
                print(rules_the_game)
                return 0
        elif choice_you == 2:
            player_first, player_second = tictactoe_human_and_human()
        elif choice_you == -1:
            return 0
        elif choice_you == 3:
            player_first, player_second = tictactoe_human_and_robot()
        elif choice_you == 4:
            player_first, player_second = tictactoe_robot_and_robot()
        game = Game(player_one=player_first, player_two=player_second)
        game.run()


def tictactoe_human_and_human():
    ...
    print(f"Первый игрок {name_second_player} играет {class_symbol_2.__name__}")
